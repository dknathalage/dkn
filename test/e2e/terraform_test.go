...
environments:
  - testenv`
	...
	mainTfContent, err := os.ReadFile(filepath.Join(componentDir, "main.tf"))
	if err != nil {
		t.Fatalf("Failed to read main.tf: %v", err)
	}

	if !contains(string(mainTfContent), "# autogenerated") {
		t.Error("main.tf should contain autogenerated comment")
	}

	if !contains(string(mainTfContent), "testcomp") {
		t.Error("main.tf should contain component name")
	}
